  ...
  asyncHandler(async (req, res) => {
    ...
    if (updatedRequest.hodStatus === "approved") {
      ...
      function drawTableRowContent(
        ...
        doc.text(columns[1], x + widths[0] + 5, y + 8, {
          width: widths[1] - 10,
        });
      }
      // Page border
      ...
      drawLabeledRow(
        "Register Number:",
        updatedRequest.student.registerNo || "N/A"
      );
      ...
      drawLabeledRow(
        ...
        `${updatedRequest.classAdvisor.name} (Class Advisor) and ${updatedRequest.hod.name} (HOD)`
      );
      drawLabeledRow("Date of Sanction:", new Date().toLocaleDateString());

      // Add time information based on timeType
      if (updatedRequest.timeType === "particularHours") {
        const startTimeStr = updatedRequest.startTime
          ? new Date(updatedRequest.startTime).toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            })
          : "N/A";
        const endTimeStr = updatedRequest.endTime
          ? new Date(updatedRequest.endTime).toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            })
          : "N/A";
      ...
      doc
        .font("Helvetica")
        .text(`Name: ${updatedRequest.classAdvisor.name}`, textOptions);
      doc.moveDown(0.5);
      ...
      doc
        .font("Helvetica")
        .text(`Name: ${updatedRequest.hod.name}`, textOptions);
      doc.moveDown(0.5);
      doc.text("", textOptions);
      doc.font("Helvetica-Bold").text("VIRTUALLY APPROVED", textOptions);
      const hodEndY = doc.y;
      ...
      doc
        .font("Helvetica")
        .text(`Name: ${updatedRequest.student.name}`, textOptions);
      doc.moveDown(0.5);
